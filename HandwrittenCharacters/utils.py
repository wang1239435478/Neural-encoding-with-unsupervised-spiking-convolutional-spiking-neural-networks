...
class fmriencoding():
    ...
    def ImageReconstruction(self, voxelidx, reploc):
        predacc = np.zeros([len(voxelidx),1])
        predfmri = np.zeros([len(voxelidx),self.testfmri.shape[1]])
        fmrival = np.zeros([14854,len(voxelidx)])
        with tqdm(total=len(voxelidx)) as pbar:
            for v in range(len(voxelidx)):
                voxel = voxelidx[v,0]
                reg = linear_model.LinearRegression()
                train_feat = self.trainfeat[:,:,np.int32(reploc[voxel,0]),np.int32(reploc[voxel,1])]
                test_feat = self.testfeat[:,:,np.int32(reploc[voxel,0]),np.int32(reploc[voxel,1])]
                reg.fit(train_feat, self.trainfmri[voxel,:].squeeze())
                predfmri[v,:] = reg.predict(test_feat)
                if ~np.any(np.isnan(self.testfmri[voxel,:])):
                    predacc[v],_ = pearsonr(predfmri[v,:], self.testfmri[voxel,:]) 
                    for i in range(14854):
                        featval = np.load('G:/SpikeCNNEncodingV8/HandwrittenCharacters/result/tichvalfeat/tichvalfeatsample'+str(i+1)+'.npy')
                        fmrival[i,v] = reg.predict(np.expand_dims(featval[0,:,np.int32(reploc[voxel,0]),np.int32(reploc[voxel,1])].squeeze(), axis=0))  
                pbar.update(1)
        return fmrival, predfmri, predacc    
